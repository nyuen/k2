resource "azurerm_resource_group" "k2_demo_rg" {
...
resource "azurerm_kubernetes_cluster" "aks_k2" {
  ...
  location            = azurerm_resource_group.k2_demo_rg.location
  resource_group_name = azurerm_resource_group.k2_demo_rg.name
  ...
  addon_profile {
    oms_agent {
      enabled                    = true
      log_analytics_workspace_id = data.azurerm_log_analytics_workspace.loganalytics_workspace.id
    }
